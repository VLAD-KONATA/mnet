...
class ContrastLoss(nn.Module):
    def __init__(self, weights=[1.0/32, 1.0/16, 1.0/8, 1.0/4, 1.0], d_func='L1', t_detach = True, is_one=False):
    ...
    def forward(self, teacher, student, neg, blur_neg=None):
        ...
        # teacher, student, neg, blur_neg=None   [B,C=3,H,W]
        blur_neg = None
...
if __name__ == '__main__':
    LOSS = ContrastLoss().cuda()
    gt = torch.rand(14,3,256,256).cuda()
    sr = torch.rand(14,3,256,256).cuda()
    lr = torch.rand(14,3,256,256).cuda()
    loss = LOSS(gt,sr,lr)
    pass
