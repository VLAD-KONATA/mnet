...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, keys=None,train=True):
        #keys = F.normalize(torch.rand((10, args.mdim), dtype=torch.float), dim=1).cuda() # Initialize the memory items
...
if __name__=='__main__':
    ...
    gpy_id = 0
    model = convAE(3,2, args.msize, args.fdim, args.mdim).cuda(gpy_id)
